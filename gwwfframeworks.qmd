...
The \acr{pn} expansion, like its name suggests, is a perturbative expansion of \acr{efe} in the orbital regime, that at \acr{lo} is given by tnewtonian gravity.  Consider a system of orbiting bodies with typical values $\bar{M},\bar{R},\bar{v}$ for masses, separations, and velocties. Then the virial theorem states:
...
We therefor expand in both velocities and coupling constant. This can be achieved with a by both a classical (albeit quite complicated) perturbative treatment [see @Blanchet:2013haa;@Blanchet:1995fg], and a more contemporary \acr{eft} based approach called coyly \acr{nrgr} [@Goldberger:2004jt,@Sturani:2021xpq] as it was inspired by \acr{nrqcd}. We call specific orders of the expansion $nPM$ when the expansion is carried out to $\order[2n]{v}$. 



Regardless of method, there are a few shortcomings with this expansion. While it is well suited to describe the *motion* of objects in an orbital regime, it is not well suited to describe gravitional radiation far from the source, as it exhibits divergences for $r\to\infty$^[in HEP speak this is an infrared divergence]. This means that in the \acr{gw} context, the \acr{pn} expansion is in fact usually coupled to a weak-field approximation (see below), outside of the near zone. This is the region surrounding the source, that is of small extent compared to the wavelength of the emitted radiation:

$$
r \ll \lambda
$$ {#eq-nearzone}

The \acr{pn} expansion can equally be viewed as an expansion in small 'retardations'. To see this consider that if the source is slowly moving, then the time derivatives of the metric generated by this source will be smaller than the spatial derivatives by a factor $\order{v}$,
$$
...
This means that retardation effects, due to the time derivative, are small corrections. In the \acr{pn} expansion we are consequently trying to compute some quantity $F(t-r)$, such as a given component of the metric, which is intrinsically a function of retarded time $t_r=t-r$, from its expansion for small retardation $r$,
$$
F(t_r)=F(t)-{r} \dot{F}(t)+\frac{r^2}{2} \ddot{F}(t)+\ldots
$$

Now importantly the derivatives should bring down factors proportional to frequency $\Omega$ of the emitted radiation. Thus this is in fact an expansion in $r \Omega\sim r / \lambda$, and is thus only valid in the near zone as defined by @eq-nearzone, and breaks down outside of this zone, in the so called *radiation zone*. A serious consequence of this fact is that the pure, naive \acr{pn} expansion is unable to account for outgoing or incoming radiation, and thus unable to describe isolated systems. This is where the \acr{pm} expansion comes in.


### \acr{pm}
 
The \acr{pm} expansion is a rather more straightforward weak field expansion of \acr{efe}, a perturbative expansion in the coupling constant $\grav$. It thus covers all orders in velocity of the \acr{pn} expansion, at a specific $\grav$ order. We usually call $nPM$ the corresponding $\order[n]{\grav}$ expansion. Historically, it was mainly seen as the radiation zone partner to the \acr{pn} expansion, providing the propagatation of the waves up to the observer. In such combination, the \acr{pm} expansion is solving the vacuum \acr{efe}. It is usally set up to compute the a version of the metric perturabation, that is exact:^[we have used $\weakmet$ to denote the linear perturbation, whereas $\pertmet$ is the all order metric perturbation.]

$$\pertmet=\sqrt{-\metricTensor} \invmet-\invmink.$$


The \acr{pm} expansion is then given as:
...
The \acr{efe} for this metric perturbation, when imposing dedonder gauge (@eq-dedonder) is given by:
...
where $\ct[\mu\nu]{\Lambda}$ is the result of putting all the nonlinear terms of the einstein tensor to the \acr{rhs}. It is dependent on the metric and thus depends highly non-linearly on $\pertmet$. We can write it as:
...
This method can be supplemented by a multipolar expansion of the $\invpertnmet{n}$, that are truncated to a specific order corresponding to the \acr{pm} order we are interested in. This combined expansion was pioneered by @Bonnor:1959, then formalised by @Thorne:1980ru, and finally applied in this present context by @Blanchet:1986. 


In terms of source dynamics, it is more appropriate (and uniquely able) to describe inherently relativistic phenomena, like gravitional scattering. Due to being less restrictive than \acr{pn} it is also relatively more difficult, as at higher orders one encounters intricate families of relativistic integrals. Note that we have in fact computed the  1\acr{pm} approximation already, in the last chapter when we linearized the \acr{efe}. Higher orders are far more difficult, especially for sources such as \acr{bbh}. 

With the rise of amplitude techniques from high energy physics, both \acr{eft} and full theory results have been succesful in extracting previously unreachable orders. nthis is because such techniques are perfectly suited to compute relativistic scattering as they were devellopped for this specific task. We will see this in the next chapter. 
...
As it's name suggests, the \acr{eob} framework tries to combine all the previous data into a hamitonian describing the motion of a single body. The idea comes from Newtonian binaries where the method to solve the two body problem is to turn it into a one body problem. Further inspiration came from the two body problem in \acr{qed} [see @Brezin:1970zr] .
...
where $A,\bar{D},Q$ are the so-called \acr{eob} potentials. $A,\bar{D}$ correspond to the radial potentials of the effective metric, while $Q$ contains terms quadratic in the momentum. We can then match these to \acr{pn} reslults, by matching observables. 

This is more than a neat trick as this Hamiltonian when well-matched fits the gold standard \acr{nr} result up to the merger, well out of the \acr{pn} regime. Crucially this Hamiltonian framework does not take into account radiation reaction by construction (since it is based on conserved energy). These dissipative effects are rather added after the fact, as friction terms in the \acr{eom}. The normal conservative eom is straightforwardly obtained through Hamiltions equations:
...
We can now account for such dissipative effects by matching to \acr{pn} results for the angular momentum flux, and the energy flux given by the so called balance laws:
...
With this we obtain as full of a picture of the inspiral as possible given the inputs, namely \acr{pn}. The matching can also be done with \acr{pm} and \acr{nr} results, further increasing the validity and precision of \acr{eob}. Of course we are still missing the merger and ringdown. The merger, or plunge was predicted in @Buonanno:2000ef to be simply the peak in the waveform. This waveform is conected smoothly to a ringdown waveform, defined by a sum of quasi -normal modes of the remnant after merger. Thus one obtains a sort of frankenstein waveform, pooling resources from \acr{pn}, \acr{pm}, \acr{nr} and black hole perturbation theory. Importantly, when well matched, this waveform is almost as accurate as \acr{nr} results, but with the added benefit of being able to be computed in a reasonable amount of time. Of course it is still only as powerful as the inputs tow which we match, namely the \acr{pn} or \acr{pm} results. We will explore how to compute \acr{pm} results using amplitudes in the next section.
