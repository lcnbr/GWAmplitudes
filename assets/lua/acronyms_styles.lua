...
styles["long-short"] = function(acronym, insert_links)
    ...
    local name = create_element(acronym.shortname, acronym.key, insert_links)
    local short
    if FORMAT:match 'html' then
      short = {pandoc.RawInline('html', '<abbr title="' .. htmlescape(acronym.longname) .. '">'),name, pandoc.RawInline('html', '</abbr>')}
    else
      short = pandoc.Span(name, { class="glossary-item", definition=acronym.longname })
    end
    local a = {}
    if acronym:isFirstUse() then
        table.insert(a, pandoc.Str(acronym.longname .. " ("))
        for i, v in ipairs(short) do
          table.insert(a, v)
        end
        table.insert(a, pandoc.Str(")"))
        text = a
    else
        text = short
    end
