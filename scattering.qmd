...
Consider for example a nonrelativistic wavefunction:$\Exp{}$S
...
$$\Debta O=\AAngle{\int \d {\overline{\Psi}(\bar{q})}\exp\pa[\big]{-\iunit\bar{q}^\mu b_\mu}\pa[\Big]{\vIntp{O}+\rIntb{O}}}  $$
...
$$\Delta p_1^{\mu,(0)}=\AAngle{\int \d {\overline{\Psi}(\bar{q})}\exp\pa[\big]{-\iunit\bar{q}^\mu b_\mu}\ \vIntb{p_1^{\mu},L=0}} $$
