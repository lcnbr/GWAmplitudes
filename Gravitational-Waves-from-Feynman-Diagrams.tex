...
eq.~\ref{eq-jacobi2}. Finally the new hamitonian
...
eq.~\ref{eq-Schwarzschild}, i.e.~we consider the case of non spinnning
...
set \(\theta=\Half{\pi}\) to fix the orientation\sidenote{\footnotesize this is always
...
given by eq.~\ref{eq-schwarzmassshell}:
...
in the classical limit:\sidenote{\footnotesize Compare to
  ...
  classical. Changing to the special kinematic variables
  eq.~\ref{eq-newmom} we can even eliminate the delta functions, since
  by constuction \(\semom[1]\cdot\tw=0\).}
