...
I would like to thank my supervisor, Prof.~Babis Anastasiou, for his
trust, his support, and kindness throughout the last half year that has
led to this thesis. Without his curiosity towards this new subject, I
would have never discovered and learned about this wonderful world at
the confluence of two great physical theories, \gls{gr} and \gls{qft} .
I would also like to extend this thanks to the rest of the theoretical
particle physics group a ETH. Dr Achilleas Lazopoulos, Dr.~Ben Ruijl,
Dr.~Valentin Hirschi, Rayan Haindl, Dario Kermanschah, Zeno Capatti,
Matilde Vicini, and Julia Karlen, the many shared lunches, coffee
breaks, and discussions have been a incredible source of inspiration and
motivation for me. In particular, I would like to thank Ben for his help
with all things \texttt{FORM}, Zeno for our very insightful discussions
and your great suggestions, and Julia for reading through the first
...
~{[}\protect\hyperlink{ref-Neill:2013wsa}{7}{]}. This is seemingly an
impossibility as \gls{qft} and \gls{gr} have historically been the two
irreconcilable theories that `together' span the whole range of scales
in the universe. \gls{qft} and the standard model, describe physics at
the smallest scales, where quantum effects dominate. This is the physics
of colliders, and condensed matter. The standard model is our best guess
as to the nature of the fundamental constituents of `matter' and
`light'.
...
differential equations ~{[}\protect\hyperlink{ref-Kotikov:1990kg}{11}{]}
and more recently generalized unitarity
~{[}\protect\hyperlink{ref-Anastasiou:2006jv}{15}{]}, all yielding
efficient computations for scattering amplitudes, can all be applied to
...
explore what conditions have to be met for them to become observable.
...
The \gls{lhs} of this equation, contains several objects that are all
only really dependent on the metric \(\met\). \(\RicciTco\) is the Ricci
tensor\sidenote{\footnotesize The Ricci Tensor is given by: \[
...
sort. The second term on the \gls{lhs} is the Ricci scalar, a further
contraction of the Ricci Tensor: \(\RicciS = \invmet \RicciTco\). The
final term on the \gls{lhs} is just a proportional constant factor of
the metric, where \(\cosmo\) is called the cosmological constant. It can
be measured and has a low known upper bound. It is in part responsible
for the expansion of the universe.

The \gls{rhs} of \ref{eq-EFE} encodes the effect of mass and energy on
the metric. \(\SEco\) is the stress-energy tensor, dependent on the
dynamics of the system. In empty space this term is zero. It is further
multiplied by the Gravitational constant \(\grav\).
...
the behavior of the source of \glspl{gw}. One could also plug
...
~{[}\protect\hyperlink{ref-Weinberg:1972}{16}{]}). Let us look at its
...
~{[}\protect\hyperlink{ref-Carroll:2019}{17}{]}). This then extends to
...
Thus any object that is modifying its shape is a source of \glspl{gw}.
All orbiting systems therefore are sources of \glspl{gw} . However as
said just above, only very important `changes in shape' have a chance to
be detectable. These phenomena are what we will explore next.
...
lose energy\sidenote{\footnotesize of course this happens in every orbiting system
  just on a time scale that is neglible. Only systems which are massive
  enough to produce large amounts of radiation actually lose enough
  energy for it to matter}. Thus they slow down, and their orbit
...
body \sidenote{\footnotesize This choice of wording is nota coincidence as we shall
  see} of reduced mass \(\redmass=\frac{\mass[1]\mass[2]}{\totmass}\)
...
mass:\sidenote{\footnotesize or equivalently between conserved angular momentum and
...
galaxies\sidenote{\footnotesize more massive objects have longer orbital periods}.
...
presented in ~{[}\protect\hyperlink{ref-Pretorius:2005gq}{25}{]}}
...
perturabation, that is exact:\sidenote{\footnotesize we have used \(\weakmet\) to
  denote the linear perturbation, whereas \(\pertmet\) is the all order
  metric perturbation.}
...
The \gls{eob} framework takes the above derivation and generalises it to
...
motion the \gls{com} energy \(E_{\text{\small COM}}=\ct[0]{P}\), where
\(P\) it the 4-momentum of the entire-system, is equal to the
hamiltonian. We have then:\sidenote{\footnotesize In the \gls{com} frame, the spacial
  part of the total momentum is zero (by defintion), and thus we have:
  \[
...
and was extended to local observables\sidenote{\footnotesize local as in not time
  integrated and thus presenting time dependent dynamics} such as
...
\gls{com} energy of the two inital states:\sidenote{\footnotesize which is equal to
  the outgoing energy}
...
\gls{eft}:\sidenote{\footnotesize here the repartioning mentioned above has been
  applied and a change in loop momenta:
  \(\tv[i]{k}\to\tv{p}+\tv[i]{\ell}\)}
...
given by applying the Heisenberg \glspl{eom} \sidenote{\footnotesize \(\odv{A_H(t)}{t}=\im\com{H_H}{A_H(t)}\)},
...
i.e.~at asymptotic times \(t\to\pm\infty\)\sidenote{\footnotesize see
  ~{[}\protect\hyperlink{ref-Coleman:2018a}{51}{]} or
  ~{[}\protect\hyperlink{ref-Collins:2019ozc}{53}{]} for a more in depth
...
in the ~{[}\protect\hyperlink{ref-Bern:2021xze}{49}{]}
...
\(\frac{\kappa}{\sqrt{\hbar}}\)\sidenote{\footnotesize as mentioned in the last
  section, this is true for gravity and \gls{sqed} and we will extend
  this fact to schematically rescale the vertex coupling by
...
Now we see that the \gls{lo} contribution \sidenote{\footnotesize Here the expansion
  ...
  \gls{lo} corresponds to \(\coupling\)}to the impulse
...
\FunctionTok{println}\NormalTok{(}\StringTok{"::: \{\#fig{-}oneloopsqed  layout{-}ncol=3\}"}\NormalTok{)}
...
{\centering \includegraphics[width=1.5in,height=3.5in]{./scattering_files/figure-latex/dot-figure-9.png}
...
{\centering \includegraphics[width=1.5in,height=3.5in]{./scattering_files/figure-latex/dot-figure-8.png}
...
{\centering \includegraphics[width=1.5in,height=3.5in]{./scattering_files/figure-latex/dot-figure-7.png}
...
\FunctionTok{println}\NormalTok{(}\StringTok{"::: \{\#fig{-}onefilterloopsqed  layout{-}ncol=5 fig{-}cap{-}location=margin\}"}\NormalTok{)}
...
{\centering \includegraphics[width=1in,height=3.5in]{./scattering_files/figure-latex/dot-figure-4.png}
...
{\centering \includegraphics[width=1in,height=3.5in]{./scattering_files/figure-latex/dot-figure-3.png}
