...
particular an \(\dimreg\)-power corresponds to a logarithm\sidenote{\footnotesize The
  third term in the sum is given by
...
of the two inital states:\sidenote{\footnotesize which is equal to the outgoing energy}
...
is:\sidenote{\footnotesize Here the repartioning mentioned above has been applied and
...
mass shell. If the spacetime `wavefunction' \sidenote{\footnotesize Note that the
  concept of coordinate space wavefunction is ill defined in interacting
  theories. However for such asymptotic states, the wavefunction outside
  the bulk (on the infinte time boundary) is that of a free
