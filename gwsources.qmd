...
## Homogenous solutions
 ...
 The first step is to consider the homogeous solution, as all solutions will involve these terms. Setting $\SEco=0$ or $\srcco=0$ yields the wave equation in the absence of sources,
$$
   \SRDA \weakmet = 0 .
$${#eq-homogenous}
...
With the homogenous part of @eq-linearized accounted for, we are now ready to derive the inhomogenous part. The solution of @eq-linearized in the presence of a source term will be heavily inspired by the analogous problem in electromagnetism, where one learns that any linear differential equation:
...
where we have ingored the non-linear terms encoded in the Levi-Civita connections. Then, 
...
Let us look at frequency domain of the soludtion given in @eq-solInhomt. We have
$$\begin{aligned} 
...
Notice that the integral on the (?:rhs) is in fact the Fourier transform of the quadrupole moment tensor of the energy denisty:
