...
The \acr{rhs} of @eq-EFE encodes the effect of mass and energy on the metric. $\SEco$ is the stress-energy tensor, dependent on the dynamics of the system. In empty space this term is zero. It is further multiplied by the Gravitational constant $G$. 
...
$$ 
\RicciTco  =
\toggle{ \frac{\invmink[\alpha\rho]}{2}\left[ \ipdv{\weakmet[\rho\alpha]}{\mu\nu} + \ipdv{\weakmet}{\rho\alpha} - \ipdv{\weakmet[\mu\alpha]}{\rho\nu} - \ipdv{\weakmet[\nu\rho]}{\mu\alpha} \right]}{\linRicciTco}\endtoggle + \mathcal{O}(h^2)=\linRicciTco+ \mathcal{O}(h^2)
$${#eq-Ricciweak}
...
$$
\square_{SR} \weakmet = -16\pi \G \overbracket{(\SEco-\frac{\mink}{2}\trSE)}^{\srcco} 
$${#eq-linearized}
$$
\ipdv{\weakmixmet[\ind{\alpha}{\mu}]}{\alpha}=\frac{1}{2}\ipdv{\trweakmet}{\mu}. 
$${#eq-dedonder}
...
One could also plug @eq-Ricciweak2 into @eq-EFE, with $\cosmo=0$, and if we change to the trace reversed perturbation: $\weakTRmet=\weakmet - \frac{1}{2}\trweakmet\mink$, we get similar and simpler equations at the cost of a more complex perturbation: ^[Note that the inverse change of variables is just: $\weakmet=\weakTRmet-\frac{1}{2}\trweakTRmet\mink$.] ^[We eliminate the trace of the stress-energy tensor by using: $\RicciS= 8\pi\G\trSE$. We can write $\linRicciS=-\frac{1}{2}\SRDA\trweakTRmet$, in de Donder gauge, which is precisely the extra term dropping out of $\SRDA\weakmet$ when we express it in terms of $\weakTRmet$ ]

$$
\square_{SR} \weakTRmet = -16\pi \G \SEco
$${#eq-linearizedTR}
$$
    \ipdv[^]{\weakTRmet}{\nu}=0. 
$${#eq-dedonderTR}
...
::: {.column-margin}
One gets the parallel solution to the trace reversed equation @eq-linearizedTR by swapping $\srcT$ with $\SETensor$ and all $\weakT$ with $\weakTTR$
:::

since when we plug in @eq-hsolInhom into @eq-EFEwave
$$
    \Box_x\weakmet = 8\G\int\dd[4]{x'}\par[\big]{(\underbrace{\Box_x\greens{\ct{x}-\ct{x'}}}_{=-2\pi\,\deltafn{\ct{x}-\ct{x'}}}} \srcco(x') = -16\pi \G\srcco(x).
$$
...
$$
\weakmet(x) = 8\G\int\dd[3]{\y}\srcco(t,\y)\dd{t'}\frac{\delta(t'-(t-\abs{\x-\y}))}{2\abs{\x-\y}} 
$$
    
$$
\weakmet(\x,t) = 4\G\int\dd[3]{\y}\frac{\srcco(t-\abs{\x-\y},\y)}{\abs{\x-\y}}.
$${#eq-solInhomt}

We can interpret the solution at $(\x,t)$ above, as the perturbation due to the sumed up contributions of all the sources at the point $(\x-\y,t_r)$ on the past light cone. Put differently this will be the gravitational radiation produced by the source $\srcco$. Additionally, the form of the time argument of the source tensor, imposed by the definition of the Green's function, shows that the radiation propagates with velocity  $=1=c$. This retarded solution satisfies the harmonic coordinate condition of @eq-linearized, as required. Indeed, we have
...
$$
\begin{split}
    \ipdv{\weakmixmet}{\mu} &= 8\G\pdv{} {\ct{x}} \int\dd[4]{x'} \greens{\ct{x}-\ct{x'}} \srcmix (x') \\
&= 8\G\int\dd[4]{x'}\,\pdv{\greens{\ct{x}-\ct{x'}}}{ \ct{x}} \srcmix(x') \\
&= -8\G\int\dd[4]{x'}\pdv{\greens{\ct{x}-\ct{x'}}}{\ct{x'}}\srcmix(x') \\
    &= \underbrace{8\G\greens{\ct{x}-\ct{x'}}\kron(x')}_{=0\ \mathrm{for}\ x=x'} + 8G\int\dd[4]{x'}\,\greens{\ct{x}-\ct{x'}}\pdv{\srcmix(x')}{\ct{x'}} \\
    &= 8G\int\dd[4]{x'}\,\greens{\ct{x}-\ct{x'}} \frac{1}{2}\pdv{\trsrc(x')}{\ct{x'}} \\
    &=\quad \dots \quad \text{repeat in reverse} \\
    &= \pdv{} {\ct{x}} \brk[\Big]{8\G\int\dd[4]{x'}\,\greens{\ct{x}-\ct{x'}} \frac{1}{2}\trsrc(x')} \\
...
$$\FT[t]{\phi}{\omega}(\omega,\x)=\int \dd{t} \phi(t,\x) \exp{\im\omega t}$$
$$\invFT[\omega]{\phi}{t}(t,\x)=\int \frac{\dd{\omega}}{2\pi} \FT[t]{\phi}{\omega}(\omega,\x) \exp{- \im \omega t}$$

Let us look at the trace reversed solution, as the conservation equation for 
$$\begin{aligned} 
\FT[t]{\weakmet}{\omega}(\omega, \x) &=\int \dd{t} \weakmet(t,\x) \exp{\im\omega t}
\\ &=4\G\int\dd[3]{\y}\dd{t} \frac{\srcco(t-\abs{\x-\y},\y)}{\abs{\x-\y}}  \exp{\im\omega t}
\\ &=4\G\int\dd[3]{\y}\dd{t_r} \frac{\srcco(t_r,\y)}{\abs{\x-\y}}  \exp{\im\omega t_r}\exp{\im\omega\abs{\x-\y}}
\\ &=4\G\int\dd[3]{\y} \frac{\FT[t_r]{\srcco}{\omega}(\omega,\y)}{\abs{\x-\y}} \exp{\im\omega\abs{\x-\y}} 
\end{aligned}$${#eq-FTweakmet}

We can now apply various approximations to this form of the perturbation. The first is to consider that we look at the radiation only in the so called *wave zone*, at a distance $r=\abs{\x}$ much larger than the dimensions of the source $R=\abs{\y}_\text{max}$. Additionally we assume that $r \gg \frac{1}{\omega}$, i.e long wavelengths don't dominate. Finally we assume that $r \gg \omega R^2$, i.e. the ratio of $R$ to the wavelength is not comparable to the ratio of $r$ to $R$. Using this approximation we can write:

$$
\begin{split}
    \abs{\x-\y} &= r\left(1-2\hat{\x}\cdot\y+\frac{\x^{\prime2}}{r^2}\right)^{1/2} \\
    \abs{\x-\y} &\approx r\left(1-\frac{\hat{\x}\cdot\y}{r}\right) \quad \text{with: } \hat{\x} = \frac{\x}{r}.
\end{split}
$$
...
Then @eq-FTweakmet becomes much simpler^[the approximations all conspire to be able to neglect the $\y$ dependence of $\exp{\im\omega\abs{\x-\y}}$ in the integral]:

$$
\FT[t]{\weakmet}{\omega}(\omega, \x)=4\G\frac{\exp{\im\omega R}}{R} \int\dd[3]{\y} \FT[t]{\srcco}{\omega}(\omega,\y) 
$${#eq-FTweakmet2}

Now let us look at the fourier transform of the source term. By definition we have

$$
\FT[t]{\srcco}{\omega}(\omega,\y)=\FT[t]{\SEco}{\omega}(\omega,\y)+\half \mink\FT[t]{\trSE}{\omega}(\omega,\y)
$$
...
This equation becomes algebraic when we further fourrier transorm in $\x$-space:

$$
\ft[\x]{\SETensor}{\k}_{\mu\nu}(\ct[\alpha]{k}) =\ft[\x]{\SETensor}{\k}_{\mu\nu}(\omega,\k) = \int \dd[3]{\y} \FT[t]{\SEco}{\omega}(\omega,\y) \exp{\im\k\cdot\y}
$$
...
$$
which when multiplied by $\co[m]{x}\co[n]{x}$ and integrated over $\x$ gives^[two integrations by parts cancels the $\co[m]{x}\co[n]{x}$ term in the \acr{lhs}, since boundary terms are 0 (the source is finite) we have $$\int \dd{x} \co[m]{x}\co[n]{x} \ipdv[^]{\FT[t]{\SEco[ij]}{\omega}}{ij}=\int\dd{x} \ipdv[^]{\co[m]{x}\co[n]{x}}{ij} \FT[t]{\SEco[ij]}{\omega}$$ 
The hessian of $\co[m]{x}\co[n]{x}$ is $(\kron[^i_m]+\kron[^i_n])(\kron[^j_n]+\kron[^j_m])$, but since $\SEco$ is symmetric the integral is $$\int \dd{\y} 2\FT[t]{\SEco[mn]}{\omega}(\omega,\y) $$]:
$$
\int \dd{\x} \FT[t]{\SEco[mn]}{\omega}(\omega,\x)=-\Half[\omega] \int \dd{\x} \co[m]{x}\co[n]{x} \FT[t]{\SEco[00]}{\omega}(\omega,\x)
$$

Notice the last integral is in fact the fourrier transform of (a third of) the quadrupole moment tensor of the energy denisty^[$$q_{mn}=3\int \co[m]{x}\co[n]{x} \SEco[00](\omega,\x)$$]. We call it $\ft[t]{q}{\omega}_{mn}(\omega)$ and we can finally rewrite @eq-FTweakmet2 as:


$$
\FT[t]{\weakmet}{\omega}(\omega, \x)=-\frac{2\G\omega^2}{3}\frac{\exp{\im\omega R}}{R} \par{\ft[t]{q}{\omega}_{mn}(\omega)+\half\mink\ft[t]{q}{\omega}^n{}_{n}(\omega)}
$$
...
$$
\begin{aligned}
\weakmet(t,\x)&=-\frac{\G}{3\pi R}\int \dd{\omega} \exp{-\im\omega(t-R)} \omega^2 \par{\ft[t]{q}{\omega}_{mn}(\omega)+\half\mink\ft[t]{q}{\omega}^n{}_{n}(\omega)}\\
&=\frac{\G}{3\pi R}  \odv[2]{}{t}\int \dd{\omega} \exp{-\im\omega(t_r)} \par{\ft[t]{q}{\omega}_{mn}(\omega)+\half\mink\ft[t]{q}{\omega}^n{}_{n}(\omega)}\\
&=\frac{2\G}{3R}\odv[2]{}{t}\par{q_{mn}(t_r)+\half\mink q^n{}_n(t_r)}
\end{aligned}
$$
