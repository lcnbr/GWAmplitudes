...
$$
which when multiplied by $\co[m]{x}\co[n]{x}$ and integrated over $\tv{x}$ gives^[two integrations by parts cancel the $\co[m]{x}\co[n]{x}$ term in the \acr{lhs} and since boundary terms are 0 (the source is finite) we have $$\int \dd{x} \co[m]{x}\co[n]{x} \ipdv[^]{\FT[t]{\SEco[ij]}{\omega}}{ij}=\int\dd{x} \ipdv[^]{\co[m]{x}\co[n]{x}}{ij} \FT[t]{\SEco[ij]}{\omega}$$ 
...
Thus the \acr{eom} $\tv{r}(t)$ of the reduced mass is, in a circular orbit, in the $x-y$ plane:

$$
\tv[xy]{r}(t)= \begin{pmatrix} \dist \cos(\Omega t)\\ \dist \sin(\Omega t) \end{pmatrix} .
$$

The corresponding \acr{eom} for both masses is simply given by applying the following scaling:
...
We can straightforwardly obtain the $00$ component of the energy momentum tensor of the system^[the energy density is the same for the effective one body system and the two body system] by using the \acr{eom} of the reduced mass:   
...
Now if we consider \acr{bh}s separated by ten times their Schwarzschild radius, at cosmological distances $\sim 100 \;\mathrm{Mpc}$, we have the following scales:
...
To have any hope of detecting such systems, one would need to have detectors sensitive in the hundreds of Hertz, with a strain sensitivity of $10^{-21}$. This has been achieved at \acr{ligo} and other detectors, and has enabled the detection of such systems. 
...
Non spinning black holes are in a certain sense the simplest binary objects one could consider modelling. However they probably would never occur in nature. The nature of formations of black holes implies that they necessarily also spin. These black holes, called kerr black holes, have spin, and therefore a different metric defining the spacetime surrounding them. The modelling spinning black holes is still quite in its infancy and we will not treat it here. 
 ...
 There are naturally many other binaries that could be feasably be detected. Finite size binaries such as \acr{ns}s include very interesting tidal effects, that are not present in the \acr{bh} case. Other non binary objects could also produce detectable gravitational waves, such as Cosmic strings, corresponding to early universe topological defects creating a stochasitc \acr{gw} backround.
